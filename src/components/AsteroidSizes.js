...
import { convertToMeters, convertToThousands } from '../helpers'
  ...
  render() {
    ...
    return (
      <li style={dangerous ? {color:'red'} : {color:'#999'}}>
        ...
        <h2 className='name'>{asteroidSizes.name.slice(1, -1)}</h2>
        <p>{convertToMeters(asteroidSizes.estimated_diameter.kilometers.estimated_diameter_max)} m</p>
